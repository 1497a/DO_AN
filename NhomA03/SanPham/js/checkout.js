function displayCheckOut() {
    ...
    if (cartItems && productContainer) {
        ...
        Object.values(cartItems).map(item => {
            ...
            <li class="list-group-item d-flex justify-content-between lh-condensed">
                <div>
                    <h6 >${item.title}</h6>  
                    <small class="text-muted">${item.price.toLocaleString('de-DE')} Đ x ${item.inCart}</small>
                </div>
                <span class="text-muted">${(item.inCart*item.price).toLocaleString('de-DE')} Đ</span>
    ...
    <strong>${cartCost.toLocaleString('de-DE')} Đ</strong></li>`
